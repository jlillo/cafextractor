...
def get_them(sc,exap,ncoef,maxords=-1,startfrom=0,nsigmas=10.,mode=1,endat=-1):
    ...
    #print gfds
    #print acoefs.shape
    return acoefs, len(acoefs), rms_all
...
#    p.terminate()
    spec = []
    #print np.shape(trace_coeffs)
    for i in range(len(npars_paralel)-1):
     	# P = 0 for the non-found orders (jlillo note)
     	if np.count_nonzero(trace_coeffs[i,:]) == 0:
     		spec.append(np.zeros((2048,2048)))
     	else:
     		spec.append(PCoeff2(npars_paralel[i]))
    spec = np.array(spec)
#    print 'end obtain_P'
...
def getSpectrum2(pars):
    ...
    FinalMatrix = np.asarray(Result)                      # After the function, we convert our list to a Numpy array.  
    FinalMatrix.resize(3,size)                            # And return the array in matrix-form.
    #print 'Shape of FinalMatrix in getSpectrum2:'
    #print np.shape(FinalMatrix)
    return FinalMatrix
...
def simple_extraction(data,coefs,ext_aperture,min_extract_col,max_extract_col,npools):
    ...
    for i in range(len(npars_paralel)-1):
    	spec.append(getSimpleSpectrum2(npars_paralel[i]))
    spec = np.array(spec)
#    p = Pool(npools)
...
#    p.terminate()
    ...
    #print np.shape(coefs)
    #print 'Hello'
    #print len(npars_paralel)
    for i in range(len(npars_paralel)):
    	#print i,npars_paralel[i],np.shape(npars_paralel[i]),np.shape(spec)
    	if np.count_nonzero(coefs[i,:]) == 0:
    		tmp = np.zeros((3,2048))
    	else:
    		tmp = getSpectrum2(npars_paralel[i])
    	#print i,np.shape(tmp)
    	#if i == 83: print tmp
    	spec.append(tmp)
    	#print 'done'
    #print 'Bye'
...
def Fit_Global_Wav_Solution(pix_centers, wavelengths, orders, Wgt, p0, minlines=1000, maxrms=150, order0=89, ntotal= 70, npix=2048, Cheby=False, Inv = False,nx=5,nm=6):
    ...
    if (Cheby):
        chebs = Calculate_chebs(pix_centers, orders, Inverse=Inv,order0=order0,ntotal=ntotal,npix=npix,nx=nx,nm=nm)
        p1, cov =  scipy.optimize.leastsq(errfunc_cheb, p0, args=(chebs, wavelengths, orders, Wgt))
        residuals    = errfunc_cheb_nw(p1, chebs, wavelengths, orders)
    else:
        p1, cov = scipy.optimize.leastsq(errfunc, p0, args=(pix_centers, orders, wavelengths))
        residuals    = errfunc(p1, pix_centers, orders, wavelengths)
    ...
    while (cond):        
        ...
        N_l -= 1
        if (Cheby):
            chebs = Calculate_chebs(pix_centers[I], orders[I], Inverse=Inv,nx=nx,nm=nm,ntotal=ntotal,npix=npix,order0=order0)
            p1, cov =  scipy.optimize.leastsq(errfunc_cheb, p0, args=(chebs, wavelengths[I], orders[I], Wgt[I]))
            residuals    = errfunc_cheb_nw(p1, chebs, wavelengths[I], orders[I] )
        else:
            p1, cov = scipy.optimize.leastsq(errfunc, p0, args=(pix_centers[I], orders[I], wavelengths[I]))
            residuals    = errfunc(p1, pix_centers[I], orders[I], wavelengths[I])
    ...
    #show()
    print "\t\t\tFinal RMS per line is ", rms_ms
    print "\t\t\tNumber of lines is ", N_l
    print "\t\t\t--> Achievable RV precision is ", rms_ms/np.sqrt(N_l)

    return p1, pix_centers[I], orders[I], wavelengths[I], I, rms_ms, residuals, cov 
...
#     tf.write("format object fmt1 \"%IDLIST(1) | %OTYPELIST(S) | %RA | %SP(S)\"\n")
