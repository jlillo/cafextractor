...
ceres_pix = file2['f1']
ceres_Species = file2['f3']
ceres_int = ceres_pix*0.0+1.0

Xpix = np.arange(2048)*1.

# ===== LOOP for each order (starting at the first order with CERES information)
for oo in np.arange(norders):
	...
	for i,line in enumerate(ceres_lines[ceres_inorder]):
		...
		twoGauss = None
		diff 		= line-ceres_lines[ceres_inorder]
		sorting 	= np.argsort(np.abs(diff))
		intensity 	= ceres_int[ceres_inorder]
		int_ratio 	= intensity[sorting[1]]/intensity[sorting[0]]
		blend_line 	= ceres_lines[ceres_inorder[sorting[1]]]
		...
		#diff = np.sort(np.abs(line-ceres_lines[ceres_inorder]))
		...
		if ok == 'yes':
			...
			yes_results.append([i, ok, round(poptX[1],2), line, intensity[sorting[0]], ceres_Species[ceres_inorder[sorting[0]]], twoGauss])
		else:
			no_results.append([i, ok, round(poptX[1],2), line, intensity[sorting[0]], ceres_Species[ceres_inorder[sorting[0]]], twoGauss])
	...
	if 1:
		fyes = open('auxiliar/yes_ceres_order_'+str(order0+oo).zfill(3)+'.dat','w')
		fno  = open('auxiliar/no_ceres_order_'+str(order0+oo).zfill(3)+'.dat','w')
		f    = open('auxiliar/ceres_order_'+str(order0+oo).zfill(3)+'.dat','w')
	...
	fnoreasons = open('auxiliar/noreasons_ceres_order_'+str(order0+oo).zfill(3)+'.dat','w')
